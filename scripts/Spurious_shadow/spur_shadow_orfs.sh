...
FA="${1}" #ORFs fasta file in data/gene_prediction/TARA_OSD_GOS_malaspina_hmp_orf.fasta.gz
ADB=data/DBs/Antifam.hmm
OUTPUT=data/spurious_and_shadows/marine_hmp_info_shadow_spurious.tsv
OUTDIR=$(dirname "${OUTPUT}")
HMMER="${PWD}"/hmmer-3.1b2/bin/hmmpress

# Collect AntiFam keys/names
grep '^NAME' "${ADB}"/AntiFam.hmm | awk '{print $2}' > "${ADB}"/antifam_keys

# Prepare the DB
"${HMMER}" "${ADB}"/AntiFam.hmm
...
# ex: data/orfs_x_projects/osd_orfs.txt, data/orfs_x_projects/tara_orfs.txt, etc.
grep '^>' ${FA} > ${OUTDIR}/orfs.txt
~/R-3.4.2/bin/Rscript --vanilla ${PWD}/scripts/Spurious_shadow/shadow_orfs.r ${OUTDIR}/orfs.txt ${OUTDIR}

# Parsing of results
#Table containing cl_name orf_name cl_size orf_length
INFO=data/mmseqs_clustering/marine_hmp_db_03112017_clu_info.tsv.gz
