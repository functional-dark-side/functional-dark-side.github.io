...
DIR=data/cluster_refinement/refined
SMPL="marine_hmp"
U="${DIR}"/"${SMPL}"_refined_noannot_cl.tsv
K="${DIR}"/"${SMPL}"_refined_annot_cl.tsv
OUTDIR=data/cluster_classification
...
"${PWD}"/scripts/Cluster_classification/cluster_ffindex_files.sh  "${SMPL}" "${OUTDIRU}"/"${SMPL}"_refined_noannot_cl_ids.txt "${OUTDIRU}"/"${SMPL}"_refined_noannot_cons.fasta
...
UR=data/DBs/uniref90_db
UR_PROT=data/DBs/uniref90_prot_ids.tsv.gz

"${PWD}"/scripts/Cluster_classification/double_search.sh "${OUTUR}" "${OUTDIRU}"/"${SMPL}"_refined_noannot_cons.fasta "${UR}" "${NSLOTS}" "${OUTUR}"/"${SMPL}"_cons_uniref90.m8

rm "${OUTUR}"/tmp -rf

"${PWD}"/scripts/Cluster_classification/hypo_parser.sh "${OUTUR}"/"${SMPL}"_cons_uniref90.m8 "${UR_PROT}" "${PWD}"/scripts/unknown_grep.tsv 1.0 "${OUTUR}"/"${SMPL}"_cons_uniref_e60_hypo_1.txt
...
NR=data/DBs/nr_db
NR_PROT=data/DBs/nr.proteins.tsv.gz

"${PWD}"/scripts/Cluster_classification/double_search.sh "${OUTNR}" "${OUTUR}"/"${SMPL}"_cons_uniref90_nohits.fasta "${NR}" "${NSLOTS}" "${OUTNR}"/"${SMPL}"_cons_nr.m8

rm "${OUTNR}"/tmp -rf

"${PWD}"/scripts/Cluster_classification/hypo_parser.sh "${OUTNR}"/"${SMPL}"_cons_nr.m8 "${NR_PROT}" "${PWD}"/scripts/unknown_grep.tsv 1.0 "${OUTNR}"/"${SMPL}"_cons_nr_e60_hypo_1.txt
...
R CMD BATCH "--args ${KANNOT} ${OUTDIRK}" "${PWD}"/scripts/Cluster_classification/pfam_domain_architect_ref.r "${OUTDIRK}"/pfam_domain_architecture.Rout
