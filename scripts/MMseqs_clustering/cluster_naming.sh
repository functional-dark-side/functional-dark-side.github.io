...
join -1 2 -2 2 <(sort -k2,2 --parallel=${NSLOTS} -S20% mmseqs_reps.tsv) \
  <(sort -k2,2 --parallel=${NSLOTS} -S20% <(awk '{print $1"\t"$2}' "${CLSTR}"_cl_name_rep_size.tsv)) \
...
rm mmseqs_reps.tsv mmseqs_reps_clname.tsv
